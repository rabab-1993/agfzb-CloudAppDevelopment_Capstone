...
def login_request(request):
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            # If not, return to home page again
...
def add_review(request, dealer_id):
    model = CarModel.objects.filter(dealer_id=dealer_id).values()
    make = CarMake.objects.all().values()
    # context = {"dealer_id": dealer_id, "cars": {"model": model, "make": make}}
    context = {"dealer_id": dealer_id, "model": model, "make": make}
    if request.method == "GET":
        print(context)
